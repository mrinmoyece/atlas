"""Load profile for the Atlas API.
...
**Does not:** model quality, or anything about a real LLM provider. Atlas
...
A run against `ATLAS_PROVIDER=anthropic` would measure the provider. That is
...
API_KEY = os.environ.get("ATLAS_LOAD_TEST_KEY", "")
REPOS = ("legacy-billing", "modern-payments")
...
BUDGETS = {
    "/healthz": {"p95_ms": 50, "p99_ms": 150},
    "/v1/analyses": {"p95_ms": 2_000, "p99_ms": 4_000},
    "/v1/analyses/stream": {"p95_ms": 3_000, "p99_ms": 6_000},
}
...
class AtlasUser(HttpUser):
    ...
    def health(self) -> None:
        ...
        with self.client.get("/healthz", name="/healthz", catch_response=True) as r:
    ...
    def analyse(self) -> None:
        with self.client.post(
            "/v1/analyses",
            json={"repo": random.choice(REPOS), "pattern": random.choice(PATTERNS)},
            name="/v1/analyses",
            catch_response=True,
        ) as r:
    ...
    def stream(self) -> None:
        with self.client.post(
            "/v1/analyses/stream",
            json={"repo": random.choice(REPOS)},
            name="/v1/analyses/stream",
            stream=True,
            catch_response=True,
        ) as r:
    ...
    def unknown_repo(self) -> None:
        ...
        with self.client.post(
            ...
            name="/v1/analyses [404]",
            catch_response=True,
        ) as r:
            if r.status_code in (404, 429):
                r.success()
            else:
                r.failure(f"expected 404/429, got {r.status_code}")
...
def enforce_budgets(environment, **_kwargs) -> None:
    ...
    for name, budget in BUDGETS.items():
        entry = (
            stats.get(name, "GET") if name in ("/healthz", "/metrics") else stats.get(name, "POST")
        )
